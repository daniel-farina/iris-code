...
async fn run(args: Value) -> Result<String> {
    let pattern = args.get("pattern").and_then(|v| v.as_str()).ok_or_else(|| anyhow!("glob: missing pattern"))?.to_string();
    let root = args.get("path").and_then(|v| v.as_str()).map(|s| PathBuf::from(shellexpand::tilde(s).into_owned()))
        .unwrap_or_else(|| std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")));
    let g = globset::Glob::new(&pattern).map_err(|e| anyhow!("invalid glob: {}", e))?.compile_matcher();
    ...
    if hits.is_empty() {
        return Ok(format!("(no matches for '{}' under {})\n", pattern, root.display()));
    }
    Ok(hits.join("\n") + "\n")
}

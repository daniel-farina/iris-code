...
async fn run(args: Value) -> Result<String> {
    let pattern = args.get("pattern").and_then(|v| v.as_str()).ok_or_else(|| anyhow!("grep: missing pattern"))?.to_string();
    ...
    let glob_match = glob.as_deref().map(|g| globset::Glob::new(g).map(|gg| gg.compile_matcher())).transpose()?;
    ...
    'outer: for entry in walker {
        ...
        if let Some(m) = &glob_match {
            ...
            if !m.is_match(basename) { continue; }
        }
        if let Ok(meta) = entry.metadata() {
            if meta.len() > 8 * 1024 * 1024 { continue; }
        }
